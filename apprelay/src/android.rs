...
/// Initialize logging
#[no_mangle]
pub extern "system" fn Java_org_platform_OHttpNativeWrapper_init(
    _env: JNIEnv,
    _class: JClass,){
...
pub extern "system" fn Java_org_platform_OHttpNativeWrapper_encapsulateRequest(
    ...
    unsafe {
        let encapsulated = crate::encapsulate_request_ffi(config.as_ptr(), config.len(), msg.as_ptr(), msg.len());
        if encapsulated.is_null() {
            return -1;
        } else {
            return encapsulated as jlong
        }
